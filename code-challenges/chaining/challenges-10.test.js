...
const createServer = () => {
  ...
  app.get('/hello', (request, response) => {
    response.send('Hi! My name is Ashley.');
  });

  app.get('/aboutme', (request, response) => {
    response.send('I sure do like doggos and I think coding is just great!');
  });

  app.get('/favoritefoods', (request, response) => {
    const favFoods = ['popcorn,', 'Thai takeout', 'fruits']; 
...
------------------------------------------------------------------------------------------------ */
//count the number of target (5) by searching through the input, and return the number of them that exist

const count = (target, input) => {
  return input.reduce((accum, currentVal) => {
    return accum + currentVal.reduce((accum2, currentVal2) => {
      if (currentVal2 === target) {
        return accum2 + 1;
      } else {
        return accum2;
      }
...
------------------------------------------------------------------------------------------------ */
//input is a an array of arrays of ints, add all ints together

const totalSum = (input) => {

  const x = input.map((currentValAnArray) => {
    return currentValAnArray.reduce((accum, currentInt) => {
      return accum + currentInt;
    }, 0);
  });
  return x.reduce((accum2, current) => {
    return accum2 + current;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // return input.map(val = val.filter(num => typeof(num) === 'number' && num % 5 === 0).map(num2 => Math.pow(2, num2)));
  return input.map(val => val.filter(x => typeof(x)==='number' && x % 5 === 0).map(x => Math.pow(2,x)));
};
...
let findMaleAndFemale = (data) => {
  return data.filter(person => person.gender === 'male' || person.gender === 'female')
    .reduce((accum, current) => accum += current.name + ' and ', '')
    .slice(0, -5);
};
...
let findShortest = (data) => {
  const nameObj = data.reduce((a,b) => a.height.length <= b.height.length ? a : b);
  return nameObj.name;
};
