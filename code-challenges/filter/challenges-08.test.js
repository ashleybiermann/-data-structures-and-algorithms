...
//   const vowels = arr.filter(current => {
...
const filterStringsWithVowels= (arr) => arr.filter(current => current.match(/[aeiou]/gi));
...
const notInFirstArray = (forbiddenValues, arr) => {
  const allowedValues = arr.filter(current => {
    if(!forbiddenValues.includes(current)) {
      return true;
    } else {
      return false;
    }
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(current => current.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(current => current.baseStat > minBaseStat).map(current => current.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(current => !current.children);
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter(current => (typeof(current) === 'number'))
    .map(current => current % 2 === 0 ? 'even': 'odd');
};
