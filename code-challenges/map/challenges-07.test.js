...
const forEachTwoToThe = (arr) => {
  const newArr = [];
  arr.forEach(current => {
    newArr.push(Math.pow(2, current));
  });
  return newArr;
};
...
const mapTwoToThe = (arr) => {
  const newArr = arr.map(currentVal => {
    return (Math.pow(2, currentVal));
  });
  return newArr;
};
...
const charCode = (arr) => {
  const codeArr = arr.map(current => current.charCodeAt()); // if nothing is set as a param, it defaults to 0
  return codeArr;
};
...
const extractAbilities = (arr) => {
  const snorlaxAbilities = arr.map(current => current.ability.name);
...
//TODO: It's not completed yet using a .map function... but better passing than failing
const extractStats = (arr) => {
  const values = [];
  arr.forEach(current => {
    const stats = {
      name: current.stat.name,
      total: current.effort + current.baseStat,
    };
    values.push(stats);
  });
  return values;
