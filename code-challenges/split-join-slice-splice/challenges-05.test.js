...
const templateWithJQuery = () => {
  starWarsPeople.forEach(person => {
    const template = $('#template').clone(); // targets the element with the id of 'template'
    template.attr('id', ''); // needed to clear the id from all of the clones so that only one true 'id=template' exists
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    const firstSpace = recipe.ingredients[i].indexOf(' ');
    const secondSpace = recipe.ingredients[i].indexOf(' ', firstSpace + 1);
    const item = recipe.ingredients[i].slice(secondSpace + 1);
    result.push(item);
  }
  return result;
};
...
const removeEvenValues = (arr) => {
  ...
  arr.forEach(integer => {
    if (integer % 2 === 1) {
      oddValues.push(integer);
    }
  });
  console.log(oddValues); //TODO: not sure why this one isn't passing. My console log shows exactly what the example about says
  return oddValues;
};
...
const totalSumCSV = (str) => {
  ...
  for (let i = 0; i < numbers.length; i++) {
    total = total + parseInt(numbers[i]); // need this parseInt to keep it from becoming a string... Gah!
  }
  return total;
};
...
//   const sorted = vowels.sort();
//   const betterVs = vowels.join('');
//   const newString = str.replace(/[aeiou]/ig, '');
//   console.log(`${sorted}`);
//   return [newString, betterVs];
// };

const extractVowels = (str) => {
  const regex = /[aeiouAEIOU]/g;
  const vowels = str.match(regex).sort().join('');
  const newString = str.replace(/[aeiou]/ig, '');
  return [newString, vowels];
};
